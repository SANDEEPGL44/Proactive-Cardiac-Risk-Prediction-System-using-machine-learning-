...
class Doctor(models.Model):
    ...
    def __str__(self):
        return self.user.username if self.user else "Unknown Doctor"
...
class Admin_Helath_CSV(models.Model):
    ...
    def __str__(self):
        return self.name or "Unnamed CSV"
...
class Search_Data(models.Model):
    ...
    prob_healthy = models.FloatField(default=0.0, null=True, blank=True)
    prob_disease = models.FloatField(default=0.0, null=True, blank=True)

    def __str__(self):
        return self.patient.user.username if self.patient and self.patient.user else "Unknown Search"
